...
namespace PDFQFZ
{
    public partial class Form1
    {
        ...
        private const string InitialHelpText =
            ...
            "4、手动点击盖章，默认单击添加一个印章，多次单击可添加多个印章。\r\n" +
            "5、指定范围页盖章，每次可在指定范围内各页的相同位置添加1个印章；需要添加多个印章时，请重复操作。\r\n" +
            "6、双击预览中的印章可以删除。\r\n" +
            "7、全部位置确认后，点击“盖章”一次生成输出文件。";
        ...
        private void InitializeAdaptiveLayout()
        {
            ...
            SetOperationHint("手动点击盖章：上传 PDF 后，单击页面位置盖章；多次单击可添加多个印章，双击印章可删除。");
        ...
        private void BuildLeftLayout(Control host)
        {
            ...
            layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 199F));
            ...
            Panel actionPanel = new Panel { Dock = DockStyle.Fill, Margin = new Padding(0) };
            bt_gz.Size = new Size(96, 36);
            bt_gz.Anchor = AnchorStyles.None;
            actionPanel.Controls.Add(bt_gz);
            actionPanel.Resize += (sender, args) =>
            {
                bt_gz.Location = new Point((actionPanel.ClientSize.Width - bt_gz.Width) / 2, 5);
            };
        ...
        {
            TableLayoutPanel section = CreateSectionTable(true);
            section.Dock = DockStyle.Fill;
            section.Padding = new Padding(0);
            section.RowCount = 4;
            section.RowStyles.Clear();
            section.RowStyles.Add(new RowStyle(SizeType.Percent, 22F));
            section.RowStyles.Add(new RowStyle(SizeType.Percent, 32F));
            section.RowStyles.Add(new RowStyle(SizeType.Percent, 25F));
            section.RowStyles.Add(new RowStyle(SizeType.Percent, 21F));

            AddSettingsRow(section, 0, "数字签名", BuildSignatureSettings());
            AddSettingsRow(section, 1, "印章设置", BuildStampSettings());
            AddSettingsRow(section, 2, "骑缝章设置", BuildSeamSettings());
            AddSettingsRow(section, 3, "其他", BuildOtherSettings());
            return section;
        }

        private Control BuildSignatureSettings()
        {
            FlowLayoutPanel row = CreateFlowRow();
        ...
        private Control BuildStampSettings()
        {
            ...
            FlowLayoutPanel first = CreateFlowRow();
            ...
            FlowLayoutPanel second = CreateFlowRow();
        ...
        private void BuildRightLayout(Control host)
        {
            ...
            comboPDFlist.Dock = DockStyle.None;
            comboPDFlist.Visible = true;
            comboPDFlist.Margin = new Padding(0);
            buttonUp.AutoSize = false;
            buttonUp.Text = "上一页";
            buttonUp.TextAlign = ContentAlignment.MiddleCenter;
            buttonUp.Visible = true;
            buttonUp.Dock = DockStyle.None;
            buttonNext.AutoSize = false;
            buttonNext.Text = "下一页";
            buttonNext.TextAlign = ContentAlignment.MiddleCenter;
            buttonNext.Visible = true;
            buttonNext.Dock = DockStyle.None;
            currentPageInput = new TextBox { Dock = DockStyle.Fill, AutoSize = false, TextAlign = HorizontalAlignment.Center, Margin = new Padding(0, 1, 6, 3) };
            currentPageInput.KeyDown += CurrentPageInput_KeyDown;
            currentPageInput.Dock = DockStyle.None;
            totalPageLabel = new Label { Dock = DockStyle.None, TextAlign = ContentAlignment.MiddleLeft, Text = "/ 0 页", Margin = new Padding(0), AutoSize = false, Visible = true };
            toolbar.Controls.Add(comboPDFlist);
            toolbar.Controls.Add(buttonUp);
            toolbar.Controls.Add(buttonNext);
            toolbar.Controls.Add(currentPageInput);
            toolbar.Controls.Add(totalPageLabel);
            toolbar.Resize += PreviewToolbar_Resize;
            PreviewToolbar_Resize(toolbar, EventArgs.Empty);
        ...
        private void PreviewToolbar_Resize(object sender, EventArgs e)
        {
            if (!(sender is Panel toolbar))
            {
                return;
            }
            ...
            const int gap = 8;
            const int pageButtonGap = 14;
            const int buttonWidth = 64;
            const int pageInputWidth = 64;
            const int totalWidth = 62;
            int y = Math.Max(0, (toolbar.ClientSize.Height - controlHeight) / 2);
            int right = toolbar.ClientSize.Width;
            totalPageLabel.Bounds = new Rectangle(right - totalWidth, y, totalWidth, controlHeight);
            right -= totalWidth;
            currentPageInput.Bounds = new Rectangle(right - pageInputWidth - gap, y, pageInputWidth, controlHeight);
            right -= pageInputWidth + gap;
            right -= pageButtonGap;
            buttonNext.Bounds = new Rectangle(right - buttonWidth, y, buttonWidth, controlHeight);
            right -= buttonWidth + gap;
            buttonUp.Bounds = new Rectangle(right - buttonWidth, y, buttonWidth, controlHeight);
            right -= buttonWidth + gap;
            comboPDFlist.Bounds = new Rectangle(0, y, Math.Max(80, right), controlHeight);
        }
        ...
        private void AddSettingsRow(TableLayoutPanel section, int row, string heading, Control content)
        {
            ...
            label.Padding = new Padding(0, 7, 0, 7);
        ...
        private void UpdatePlacementOperationHint()
        {
            ...
            {
                if (comboType.SelectedIndex == 0 && Directory.Exists(pathText.Text))
                {
                    SetOperationHint("请在上方文件列表中选择目录内的 PDF 文件进行预览。");
                }
                else
                {
                    SetOperationHint("手动点击盖章：上传 PDF 后，单击页面位置盖章；多次单击可添加多个印章，双击印章可删除。");
            ...
            else if (specifiedRangeFirstClickPending && specifiedPageRange != null)
            {
                SetOperationHint("已跳转至范围内最后一页。单击完成指定范围页盖章；范围内需要再盖一个章时，请再次点击“指定范围页盖章”。");
            }
            else if (comboYz.SelectedIndex == CustomPlacementStampType)
            {
                SetOperationHint("请在右侧 PDF 页面中单击需要盖章的位置；多次单击可添加多个印章，双击印章可删除。");
            }
            else
            {
                SetOperationHint("当前未启用页面印章，可继续查看 PDF 或设置骑缝章。");
            }
        }
        ...
        private void LayoutPreviewPage()
        {
            ...
            Size size = PreviewViewportLayout.Calculate(
